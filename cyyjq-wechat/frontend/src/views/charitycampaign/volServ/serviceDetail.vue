...
<script>
import { fetchCurrentvolservice, volserviceSignup } from '@/api/volunteer'
...
export default {
  ...
  methods: {
    fetchData() {
      fetchCurrentvolservice(window.localStorage.getItem('clicktitle')).then(response => {
    ...
    handleSignup() {
      console.log('点击报名')
      volserviceSignup(this.content[0].id, window.localStorage.getItem('user_id')).then(response => { // mockserver返回20000和包在data的token，实际后端只返回token
        this.$router.push({ path: '/volServ/serviceList' })
      }).catch(error => {
        console.log(error)
        // reject(error)
      })
